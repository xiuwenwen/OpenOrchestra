...
class HarnessStateView:
    ...
    def snapshot(self, task_id: str | None = None) -> dict[str, Any]:
        ...
        for run_record in self.repository.list_agent_runs(task_id):
            ...
            enriched_run["live_path"] = self._file_info(log_dir / "live.log")
            enriched_run["trace_path"] = self._file_info(log_dir / "agent_trace.md")
            enriched_run["command_path"] = self._file_info(log_dir / "command.txt")
