...
DEFAULT_ROLE_INSTRUCTIONS = {
    ...
    "tester": (
        ...
        "Do not rely on a later Harness test gate; `tester_result.json.tester_status_code` must decide `0` tests passed, `1` source bug, or `2` environment blocked. "
        "`tester_result.json.environment_dependency_issue` must explicitly report whether dependency/setup/build/test execution is blocked by environment problems; Harness checks it before `tester_status_code`. "
        "When `selected_plan.json.acceptance_oracles` is present, verify `required_for_tester: true` oracle IDs directly and fill numeric `tester_result.json.oracle_results`; final-only oracles may be marked not-run with code `3`, not guessed as passed. Do not reinterpret the acceptance baseline from executor prose. "
