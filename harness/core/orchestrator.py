...
FINAL_COMMUNICATOR_ARTIFACTS = _types("final_delivery.md", "usage_guide.md")
...
class Orchestrator:
    ...
    def run_role_phase(
        ...
        agent_count = self._effective_agent_count(task_id, role, phase, agent_count_override)
        ...
        timeout_seconds = int(self._get_task_config(task_id)["timeouts"][role])
        backend = self._backend_for(task_id, role)
            ...
            while not stop_event.wait(interval):
                ...
                self._emit(
                    ProgressEvent(
                        ...
                        data={
                            "backend": self._backend_for(context.task_id, context.role),
                            "workspace": str(context.workspace_dir),
                            "logs": str(context.log_dir),
                            "elapsed_seconds": elapsed_seconds,
                        },
    ...
    def _get_task_config(self, task_id: str) -> dict[str, Any]:
        ...
        if task and task.get("configuration"):
            try:
                ...
                self.logger.warning(f"Failed to parse task configuration for {task_id}, using global config.")
    ...
    def _backend_for(self, task_id: str, role: str) -> str:
        config = self._get_task_config(task_id)
        return config["agent_backend"].get(role) or config["agent_backend"].get("default", "mock")
    ...
    def _effective_agent_count(self, task_id: str, role: str, phase: str, agent_count_override: int | None = None) -> int:
        ...
        return int(self._get_task_config(task_id)["roles"][role]["count"])
    ...
    def _stage_input_artifacts(
        ...
        visible_artifacts = self._filter_visible_artifacts_for_phase(
            visible_artifacts,
            phases_by_id,
            role,
            phase,
    ...
    def _filter_visible_artifacts_for_phase(
        ...
        rules = self._artifact_visibility_rules_for(role, phase)
    ...
    def _artifact_visibility_rules_for(self, role: str, phase: str) -> list[ArtifactVisibilityRule]:
    ...
    def _artifact_visible_to(self, role: str, phase: str, artifact: dict[str, Any]) -> bool:
        ...
        if role == "tester":
            ...
            return False
        return self._artifact_allowed_by_visibility_table(role, phase, artifact_role, artifact_type)

    def _artifact_allowed_by_visibility_table(
        self,
        target_role: str,
        target_phase: str,
        source_role: str,
        ...
        for rule in self._artifact_visibility_rules_for(target_role, target_phase):
