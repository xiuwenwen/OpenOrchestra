...
delivery.md is a role return envelope. It must contain `return_code: 0` when the required fix artifacts are complete. Complete fix Markdown deliverables must contain `artifact_result_code: 0`. Do not use `return_code` or `artifact_result_code` for test, review, or judge verdicts.
