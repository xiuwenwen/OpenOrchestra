...
delivery.md is a role return envelope. It must contain `return_code: 0` when the required planning artifacts are complete, even if the plan identifies high risks. Put planning peer-review outcomes only in `peer_review_code`; never copy peer-review values into `return_code` or `artifact_result_code`.
