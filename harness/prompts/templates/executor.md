...
delivery.md is a role return envelope. It must contain `return_code: 0` when the required executor artifacts are complete. Complete executor Markdown deliverables must contain `artifact_result_code: 0`. Do not use `return_code` or `artifact_result_code` for implementation quality, review verdicts, or test verdicts.
