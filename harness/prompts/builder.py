    ...
    def _phase_specific_rules(self, context: AgentRunContext) -> list[str]:
        if context.role == "judge":
            if context.phase == "TEST_JUDGEMENT":
                return [
                    ...
                    "- `decision.json` must include an `evidence` object summarizing objective gate facts, tester_result.json, changed files, and any blocking findings you relied on.",
        ...
        if context.role == "tester":
            return [
                ...
                '- Required JSON shape: `{"schema_version":1,"status":"tests_passed|source_bug|environment_blocked","next_action":"continue|fix_code|block_task","failure_type":"none|source_bug|env_setup|test_command|test","summary":"...","setup_commands_run":[],"test_commands_run":[],"remaining_blockers":[]}`.',
                '- Use `status: "tests_passed"` and `next_action: "continue"` only when the relevant setup/build/test/smoke evidence passed.',
                '- Use `status: "source_bug"` and `next_action: "fix_code"` only when commands ran in a working environment and show a source or behavior failure that executor should fix.',
        ...
        if context.role == "communicator":
            return [
                ...
                "- Use `selected_plan.md`, final executor artifacts, final `bug_report.md`, and final `tester_result.json` as your primary sources of truth. Do not pad the customer handoff with judge chatter, gate reports, or internal retry history.",
